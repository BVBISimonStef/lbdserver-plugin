...
import { lbdReferences, subjectObjectReferences, glTFReferences } from '../../pages/Documentation/Dialogs/alignments';
    ...
    return <div>
        ...
        {(project && getDefaultSession().info.isLoggedIn) ? (
            <div>
                ...
                <Button variant="outlined" onClick={align} disabled={loading}>Start Alignment of IFC and glTF</Button>
                </div>
                ) : (
                    <div>
                    <Typography>Create abstract concepts for the selected dataset</Typography>
                    <Button variant="outlined" onClick={createIdentifiers} disabled={loading}>Create concepts</Button>
                    </div>
                )}
                {error ? (
